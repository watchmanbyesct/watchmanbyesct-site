# Watchman Mail Privacy Policy

Effective Date: June 24, 2026  
Product: Watchman Mail  
Owner: ESCT Holding Inc.  
Repository: planned or module repository not located during review  
Version: watchman-mail-privacy-v1.0

## 1. Purpose

This Privacy Policy explains how ESCT Holding Inc. collects, uses, stores, shares, protects, and retains information through Watchman Mail. This document is intended for publication on the public website and for display inside the application before account activation, installation, or first use.

Watchman Mail is a secure email client and communication application.

## 2. Information We Collect

Depending on configuration and enabled features, Watchman Mail may collect or process the following categories of information:
...
2. Operational or product data, including email account identifiers, OAuth tokens or provider connection records, mailbox metadata, message headers, message content, attachments, drafts, sent messages, labels/folders, contact data, audit logs, and support records.
...
6. Payment or billing references if payment, subscription, enrollment, or invoicing features are enabled. Full payment card details should be handled by authorized payment processors and should not be stored by Watchman Mail unless explicitly approved through a secure payment architecture.

## 3. How We Use Information

ESCT Holding Inc. uses information to:

1. Provide, operate, secure, maintain, and improve Watchman Mail.
...
Retention may vary by tenant configuration, product function, legal requirement, and record type. Deleting data from Watchman Mail may not delete copies retained by third-party providers, integrations, backups, audit logs, legal holds, or tenant-controlled exports.
...
Watchman Mail is not intended for children under 13. Tenant administrators must not knowingly authorize child-directed use unless a separate child-privacy review and written legal approval have been completed.
...
1. Create a public route for this document at `/legal/watchman-mail/privacy` or `/privacy/watchman-mail`.
...
3. Store this document as `privacy.md` in `legal/watchman-mail/privacy.md` or the product repository equivalent.
4. Display the version `watchman-mail-privacy-v1.0` and effective date in the UI.
5. Require users to accept the current Privacy Policy before installation, first use, tenant enablement, provider connection, OAuth authorization, IMAP/SMTP setup, and any new mailbox permission request.
