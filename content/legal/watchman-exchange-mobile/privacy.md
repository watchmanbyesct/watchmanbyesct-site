# Watchman Exchange Mobile Privacy Policy

Effective Date: June 24, 2026  
Product: Watchman Exchange Mobile  
Owner: ESCT Holding Inc.  
Repository: watchmanbyesct/watchman-exchange-mobile  
Version: watchman-exchange-mobile-privacy-v1.0

## 1. Purpose

This Privacy Policy explains how ESCT Holding Inc. collects, uses, stores, shares, protects, and retains information through Watchman Exchange Mobile. This document is intended for publication on the public website and for display inside the application before account activation, installation, or first use.

Watchman Exchange Mobile is a mobile application for Watchman Exchange.

## 2. Information We Collect

Depending on configuration and enabled features, Watchman Exchange Mobile may collect or process the following categories of information:
...
2. Operational or product data, including mobile login identifiers, profile data, posts, comments, reactions, device identifiers, push notification tokens, media uploads, crash/error logs, and moderation records.
...
6. Payment or billing references if payment, subscription, enrollment, or invoicing features are enabled. Full payment card details should be handled by authorized payment processors and should not be stored by Watchman Exchange Mobile unless explicitly approved through a secure payment architecture.

## 3. How We Use Information

ESCT Holding Inc. uses information to:

1. Provide, operate, secure, maintain, and improve Watchman Exchange Mobile.
...
Retention may vary by tenant configuration, product function, legal requirement, and record type. Deleting data from Watchman Exchange Mobile may not delete copies retained by third-party providers, integrations, backups, audit logs, legal holds, or tenant-controlled exports.
...
Watchman Exchange Mobile is not intended for children under 13. Tenant administrators must not knowingly authorize child-directed use unless a separate child-privacy review and written legal approval have been completed.
...
1. Create a public route for this document at `/legal/watchman-exchange-mobile/privacy` or `/privacy/watchman-exchange-mobile`.
...
3. Store this document as `privacy.md` in `legal/watchman-exchange-mobile/privacy.md` or the product repository equivalent.
4. Display the version `watchman-exchange-mobile-privacy-v1.0` and effective date in the UI.
5. Require users to accept the current Privacy Policy during mobile installation, first app launch, account activation, push notification enablement, media upload, or material legal update.
