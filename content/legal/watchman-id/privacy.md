# Watchman ID Privacy Policy

Effective Date: June 24, 2026  
Product: Watchman ID  
Owner: ESCT Holding Inc.  
Repository: watchmanbyesct/watchman-id  
Version: watchman-id-privacy-v1.0

## 1. Purpose

This Privacy Policy explains how ESCT Holding Inc. collects, uses, stores, shares, protects, and retains information through Watchman ID. This document is intended for publication on the public website and for display inside the application before account activation, installation, or first use.

Watchman ID is a identification, badging, credential issuance, NFC/magstripe, and access-card management platform.

## 2. Information We Collect

Depending on configuration and enabled features, Watchman ID may collect or process the following categories of information:
...
2. Operational or product data, including badge holder profiles, photos, employee IDs, card serial numbers, NFC identifiers, magstripe encoding references, issuance dates, revocation records, access credentials, printer/device logs, and audit records.
...
6. Payment or billing references if payment, subscription, enrollment, or invoicing features are enabled. Full payment card details should be handled by authorized payment processors and should not be stored by Watchman ID unless explicitly approved through a secure payment architecture.

## 3. How We Use Information

ESCT Holding Inc. uses information to:

1. Provide, operate, secure, maintain, and improve Watchman ID.
...
Retention may vary by tenant configuration, product function, legal requirement, and record type. Deleting data from Watchman ID may not delete copies retained by third-party providers, integrations, backups, audit logs, legal holds, or tenant-controlled exports.
...
Watchman ID is not intended for children under 13. Tenant administrators must not knowingly authorize child-directed use unless a separate child-privacy review and written legal approval have been completed.
...
1. Create a public route for this document at `/legal/watchman-id/privacy` or `/privacy/watchman-id`.
...
3. Store this document as `privacy.md` in `legal/watchman-id/privacy.md` or the product repository equivalent.
4. Display the version `watchman-id-privacy-v1.0` and effective date in the UI.
5. Require users to accept the current Privacy Policy before tenant setup, badge issuance, photo capture, card encoding, credential assignment, or hardware integration activation.
