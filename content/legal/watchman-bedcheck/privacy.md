# Watchman BedCheck Privacy Policy

Effective Date: June 24, 2026  
Product: Watchman BedCheck  
Owner: ESCT Holding Inc.  
Repository: watchmanbyesct/watchman-bedcheck  
Version: watchman-bedcheck-privacy-v1.0

## 1. Purpose

This Privacy Policy explains how ESCT Holding Inc. collects, uses, stores, shares, protects, and retains information through Watchman BedCheck. This document is intended for publication on the public website and for display inside the application before account activation, installation, or first use.

Watchman BedCheck is a NFC-verified welfare rounds and bed check platform.

## 2. Information We Collect

Depending on configuration and enabled features, Watchman BedCheck may collect or process the following categories of information:
...
2. Operational or product data, including facility records, room numbers, resident identifiers or coded IDs, round schedules, NFC scan events, welfare check notes, exceptions, reports, device metadata, and audit logs.
...
6. Payment or billing references if payment, subscription, enrollment, or invoicing features are enabled. Full payment card details should be handled by authorized payment processors and should not be stored by Watchman BedCheck unless explicitly approved through a secure payment architecture.

## 3. How We Use Information

ESCT Holding Inc. uses information to:

1. Provide, operate, secure, maintain, and improve Watchman BedCheck.
...
Retention may vary by tenant configuration, product function, legal requirement, and record type. Deleting data from Watchman BedCheck may not delete copies retained by third-party providers, integrations, backups, audit logs, legal holds, or tenant-controlled exports.
...
Watchman BedCheck is not intended for children under 13. Tenant administrators must not knowingly authorize child-directed use unless a separate child-privacy review and written legal approval have been completed.
...
1. Create a public route for this document at `/legal/watchman-bedcheck/privacy` or `/privacy/watchman-bedcheck`.
...
3. Store this document as `privacy.md` in `legal/watchman-bedcheck/privacy.md` or the product repository equivalent.
4. Display the version `watchman-bedcheck-privacy-v1.0` and effective date in the UI.
5. Require users to accept the current Privacy Policy before facility setup, device deployment, resident/room record creation, scan activity, report generation, or staff mobile installation.
