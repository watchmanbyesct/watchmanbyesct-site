# Watchman Exchange Privacy Policy

Effective Date: June 24, 2026  
Product: Watchman Exchange  
Owner: ESCT Holding Inc.  
Repository: watchmanbyesct/watchman-exchange  
Version: watchman-exchange-privacy-v1.0

## 1. Purpose

This Privacy Policy explains how ESCT Holding Inc. collects, uses, stores, shares, protects, and retains information through Watchman Exchange. This document is intended for publication on the public website and for display inside the application before account activation, installation, or first use.

Watchman Exchange is a employee-only social, engagement, and communication platform.

## 2. Information We Collect

Depending on configuration and enabled features, Watchman Exchange may collect or process the following categories of information:
...
2. Operational or product data, including user profiles, posts, comments, reactions, media uploads, employment/tenant eligibility status, moderation records, reports, notifications, and audit logs.
...
6. Payment or billing references if payment, subscription, enrollment, or invoicing features are enabled. Full payment card details should be handled by authorized payment processors and should not be stored by Watchman Exchange unless explicitly approved through a secure payment architecture.

## 3. How We Use Information

ESCT Holding Inc. uses information to:

1. Provide, operate, secure, maintain, and improve Watchman Exchange.
...
Retention may vary by tenant configuration, product function, legal requirement, and record type. Deleting data from Watchman Exchange may not delete copies retained by third-party providers, integrations, backups, audit logs, legal holds, or tenant-controlled exports.
...
Watchman Exchange is not intended for children under 13. Tenant administrators must not knowingly authorize child-directed use unless a separate child-privacy review and written legal approval have been completed.
...
1. Create a public route for this document at `/legal/watchman-exchange/privacy` or `/privacy/watchman-exchange`.
...
3. Store this document as `privacy.md` in `legal/watchman-exchange/privacy.md` or the product repository equivalent.
4. Display the version `watchman-exchange-privacy-v1.0` and effective date in the UI.
5. Require users to accept the current Privacy Policy before first access, profile activation, posting, commenting, media upload, or mobile app installation.
